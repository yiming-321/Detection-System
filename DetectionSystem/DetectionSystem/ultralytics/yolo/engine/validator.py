        ...
        for batch_i, batch in enumerate(bar):
            ...
            with dt[0]:
                ...
                if self.args.myscale==1:
                    batch = self.preprocess(batch)
                    batch['img']=torch.stack([batch['img']],dim=-1)
                if self.args.myscale==2:
                    batch = self.preprocess(batch)
                    batch['img']=torch.stack([batch['img'],batch['img']],dim=-1)
                if self.args.myscale==5:
                    batch = self.preprocess(batch)
                    batch['img']=torch.stack([batch['img'],batch['img'],batch['img'],batch['img'],batch['img']],dim=-1)
